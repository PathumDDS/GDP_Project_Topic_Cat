...
# --- CONFIGURATION ---
MODEL_FILE = "prediction/data_weekly/srilanka_gdp_model.pkl"
WEEKLY_DATA_FILE = "prediction/data_weekly/weekly_preprocessed_data.csv"  # Raw Weekly Keywords
ORIGINAL_DATA_FILE = "prediction/data_weekly/gdp_merged_data.csv" # Used to recover PCA map
GDP_FILE = "prediction/data_weekly/gdp_sri_lanka.csv" # Actual GDP for plotting
...
def run_tracker():
    ...
    # We create 13 weeks of future dates so the shifted data has a place to go
    last_date = tracker.index[-1]# Force frequency to 'W' (Weekly)
    future_dates = pd.date_range(start=last_date + pd.Timedelta(weeks=1), periods=13, freq='W')    
    # Add these empty rows to the dataframe
    tracker = tracker.reindex(tracker.index.union(future_dates))
    ...
    plt.tight_layout()
    plt.savefig("prediction/data_weekly/WEEKLY_TRACKER_RESULT.png")
    tracker.to_csv("prediction/data_weekly/final_tracker_values.csv")
    print("Success! Tracker saved to: prediction/data_weekly/WEEKLY_TRACKER_RESULT.png")
